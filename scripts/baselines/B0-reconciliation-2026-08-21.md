...
Extract `src/services/lake-visibility.js` (`lakeIndexableSql` + `lakeRobots`), reflecting **all** content columns (intro_text / description / unique lifestyle_text / seasons_text), used by the sitemap query, the `/lakes/:slug` route, **and** the link surfaces — mirroring `town-visibility.js`. Add a **gate-parity test** asserting sitemap-gate == robots-gate == link-surface-gate, so neither the orphan (town) nor the linked-but-noindex (lake) direction can recur.
